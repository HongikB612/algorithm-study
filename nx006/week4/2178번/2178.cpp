...
struct Position {
  ...
  // map의 key값으로 이용할 거라서, 정렬을 위한 비교 연산자 정의해주어야 함.
...
void backtracking(map<Position, Position>& moves, Position& trackStartPoint) {
  ...
  auto numberOfVisited = 0;
  while (!list.empty()) {
    numberOfVisited++;
    list.pop();
  }
  std::cout << numberOfVisited << std::endl;
}
/*********
...
void ShortestPath(const int m, const int p)
{
  ...
  std::vector<std::vector<bool> > mark_short(MAXSIZE + 1);
  for (int i = 0; i < MAXSIZE + 1; i++) {
    mark_short[i].resize(MAXSIZE + 1);
  }

  //bool** mark_frontier = new bool* [MAXSIZE + 1]; ///> frontier queue에 이미 추가된 장소를 표시. 이미 queue에 대기중이라면 애써 또 추가할 필요 없다
  std::vector<std::vector<bool> > mark_frontier(MAXSIZE + 1);
  for (auto& row : mark_frontier) {
    row.resize(MAXSIZE +1);
  }

  // 함수 구현에 3가지가 필요함.
  // current -> 현재 위치
  Position current(1, 1, EAST); ///> 시작 위치로 초기화한 현재 위치
  ...
  while (!frontier.empty()) {
    ...
    for (int d = 0; d < 4; d++) { // N방향부터 4개 방위를 전부 검사함
